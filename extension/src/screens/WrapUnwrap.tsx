...
export default function WrapUnwrap({ address: _address, privateKey, initialToken, onBack }: Props) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const withNative = [NATIVE_TOKEN_METADATA, ...customs.filter(
        (t) => !isNativeTokenAddress(t.address),
      )];
      setTokens(withNative);

      if (initialToken) {
        if (isNativeTokenAddress(initialToken.address)) {
          setSelectedToken(NATIVE_TOKEN_METADATA);
        } else {
          const match = customs.find(t => t.address.toLowerCase() === initialToken.address.toLowerCase());
          setSelectedToken(match || initialToken);
        }
      } else {
        setSelectedToken(withNative[0]);
      }
    })();
  }, [initialToken]);
  ...
  useEffect(() => {
    ...
    if (isNativeTokenAddress(selectedToken.address)) {
      const network = getActiveNetwork();
      setWrapperReady(isNativeWrapperConfigured(network.id));
      return;
    }
    if (!registryConfigured) { setWrapperReady(null); return; }
    let cancelled = false;
    (async () => {
      try {
  ...
  const fetchBal = async () => {
    ...
    try {
      const provider = getProvider();
      if (isNativeTokenAddress(selectedToken.address)) {
        const bal = await provider.getBalance(_address);
      ...
      setPublicBalance(formatUnitsDisplay(bal, decimals));
  ...
  const handleWrap = async () => {
    ...
    const isNative = isNativeTokenAddress(selectedToken.address);

    if (isNative) {
      if (!isNativeWrapperConfigured(getActiveNetwork().id)) {
        throw new Error('Native ETH wrapper not configured on this network');
      }
      setStatusMsg('Shielding ETH to cETH (FHERC20)...');
      wrapTx = await shieldNative(signer, _address, parsedAmount);
      setHashes({ action: wrapTx.hash });
    } else {
    ...
    const network = getActiveNetwork();
    const confidentialSymbol = isNative ? 'cETH' : `c${selectedToken.symbol}`;
    ...
    await addActivity({
      ...
      tokenSymbol: selectedToken.symbol,
      tokenAddress: isNative ? NATIVE_TOKEN_METADATA.address : selectedToken.address,
      chainId: network.chainId,
      txStage: isNative ? 'shield-native-submitted' : 'shield-submitted',
    });
    ...
    await addActivity({
      ...
      tokenAddress: isNative ? NATIVE_TOKEN_METADATA.address : selectedToken.address,
      chainId: network.chainId,
      txStage: isNative ? 'shield-native-confirmed' : 'shield-confirmed',
    });
  };
    ...
    <div className="ui-density-comfortable w-full min-h-screen overflow-hidden bg-app text-main font-sans relative flex flex-col">
      ...
      <main className="flex-1 w-full px-6 pt-6 relative z-10 overflow-y-auto no-scrollbar pb-4">
        ...
        <div className="flex-1 space-y-8">
          {!registryConfigured && selectedToken && !isNativeTokenAddress(selectedToken.address) && (
            <div className="p-3 border-l-4 border-red-500 bg-red-500/5 text-caption font-bold uppercase tracking-widest text-red-500">
              Registry not configured — deploy FHERC20WrapperRegistry and update the extension.
            </div>
          )}
          {selectedToken && isNativeTokenAddress(selectedToken.address) && !isNativeWrapperConfigured(getActiveNetwork().id) && (
            <div className="p-3 border-l-4 border-yellow-500 bg-yellow-500/5 text-caption font-bold uppercase tracking-widest text-yellow-500">
              Native ETH wrapper not set for this network. Deploy FHERC20NativeUnderlyingWrapper (see hardhat) and add the address to nativeToken.ts or VITE_NATIVE_WRAPPER_* env.
            </div>
          )}

          {/* Token Details */}
          {selectedToken && (
            <div className="p-4 bg-surface border-2 border-ui space-y-4">
              <div className="flex items-center gap-4">
                <TokenIcon symbol={selectedToken.symbol} className="w-12 h-12 shrink-0 border border-ui bg-app overflow-hidden" />
                <div className="flex-1 min-w-0">
                  <div className="font-brand font-bold text-lg tracking-tight">{selectedToken.symbol}</div>
                  <div className="text-caption font-mono text-muted truncate">
                    {isNativeTokenAddress(selectedToken.address)
                      ? 'Native asset · shieldNative'
                      : `${selectedToken.address.slice(0, 6)}...${selectedToken.address.slice(-4)}`}
                  </div>
                </div>
                {wrapperReady !== null && (
                  <div className={`text-micro font-bold uppercase tracking-widest ${wrapperReady ? 'text-brand-cyan' : 'text-yellow-500'}`}>
                    {wrapperReady
                      ? 'Wrapper Ready'
                      : isNativeTokenAddress(selectedToken.address)
                        ? 'Not Deployed'
              ...
              {/* Balance display */}
              <div className="flex items-center justify-between pt-3 border-t-2 border-ui">
                {mode === 'wrap' ? (
                  <>
                    <div className="text-caption font-bold text-muted uppercase tracking-widest">Available Balance</div>
                  ...
                  <>
                    <div className="text-caption font-bold text-muted uppercase tracking-widest flex items-center gap-1.5">
                    ...
                    {privateBalance !== null ? (
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 text-caption font-bold text-brand-cyan hover:text-brand-cyan/80 uppercase tracking-widest transition-colors"
                      >
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-4 bg-input-field p-4 border border-ui relative">
               <div className="flex-1 text-center py-2 bg-app text-label-caps text-caption text-sub">
          ...
          <div className="space-y-3">
            ...
            {mode === 'unwrap' && (
              <div className="text-micro text-muted font-mono leading-relaxed uppercase opacity-60 px-1">
                Sequence: Unshield → DecryptForTx → Claim
              </div>
            )}
            {mode === 'wrap' && selectedToken && isNativeTokenAddress(selectedToken.address) && (
              <div className="text-micro text-muted font-mono leading-relaxed uppercase opacity-60 px-1">
                Native wrap uses shieldNative; amounts align to wrapper rate() (dust refunded).
              </div>
            )}
          </div>
                ...
                <div className="mt-3 space-y-2 pt-3 border-t border-brand-cyan/10">
                  {hashes.approve && (
                    <a 
                      ...
                      className="flex items-center justify-between text-detail font-mono text-main hover:text-brand-cyan transition-colors group"
                    >
                      <span className="font-bold uppercase tracking-widest text-brand-cyan">Step 1: Hash</span>
                      <div className="flex items-center gap-1.5 bg-brand-cyan/10 px-2 py-0.5 border border-brand-cyan/35 group-hover:bg-brand-cyan/30">
        ...
        {status === 'success' ? (
          ...
          <button
            ...
            disabled={
              ...
              || parseFloat(amount) <= 0
              || status === 'loading'
              || (selectedToken && isNativeTokenAddress(selectedToken.address)
                ? !isNativeWrapperConfigured(getActiveNetwork().id)
                : !registryConfigured)
            }
